...
#define STATUS_NO_MATCH 0xC000001E
#define STATUS_ACCESS_DENIED 0xC0000022
#define STATUS_BUFFER_TOO_SMALL 0xC0000023
#define STATUS_OBJECT_TYPE_MISMATCH 0xC0000024
#define STATUS_FILE_NOT_FOUND 0xC0000028
...
typedef NTSTATUS (WINAPI *LPFN_NTOPENFILE)(
	PHANDLE FileHandle,
...
static LPFN_NTOPENFILE NtOpenFile = NULL;

typedef NTSTATUS (WINAPI *LPFN_NTREADFILE)(
	HANDLE FileHandle,
	HANDLE Event,
...
static LPFN_NTREADFILE NtReadFile = NULL;

typedef NTSTATUS (WINAPI *LPFN_NTWRITEFILE)(
	HANDLE FileHandle,
	HANDLE Event,
	PIO_APC_ROUTINE ApcRoutine,
	PVOID ApcContext,
...
static LPFN_NTWRITEFILE NtWriteFile = NULL;

typedef NTSTATUS (WINAPI *LPFN_NTCLOSE)(
	HANDLE Handle
);
static LPFN_NTCLOSE NtClose = NULL;
...
static LPFN_NTDEVICEIOCONTROLFILE NtDeviceIoControlFile = NULL;

typedef NTSTATUS (WINAPI *LPFN_NTFSCONTROLFILE)(
	HANDLE FileHandle,
	HANDLE Event,
	PIO_APC_ROUTINE ApcRoutine,
	PVOID ApcContext,
...
static LPFN_NTFSCONTROLFILE NtFsControlFile = NULL;
...
typedef struct {
	...
	HANDLE vol_handle;
	BOOL ntdll;
	BOOL noclose;		/* Don't close the handle when unmount */
} win32_fd;
...
{
	ntfs_log_trace("Converting ntstatus 0x%x.\n",status);
	switch (status) {
...
static void ntfs_device_win32_init_imports(void)
{
	...
	if (!ntdll) {
		...
		ntfs_log_trace("ntdll.dll could not be imported.\n");
	}
	if (!NtOpenFile && ntdll)
		NtOpenFile = (LPFN_NTOPENFILE)
				GetProcAddress(ntdll,
				"NtOpenFile");
	if (!NtReadFile && ntdll)
		NtReadFile = (LPFN_NTREADFILE)
				GetProcAddress(ntdll,
				"NtReadFile");
	if (!NtWriteFile && ntdll)
		NtWriteFile = (LPFN_NTWRITEFILE)
				GetProcAddress(ntdll,
				"NtWriteFile");
	if (!NtClose && ntdll)
		NtClose = (LPFN_NTCLOSE)
				GetProcAddress(ntdll,
				"NtClose");
	if (!NtDeviceIoControlFile && ntdll)
		NtDeviceIoControlFile = (LPFN_NTDEVICEIOCONTROLFILE)
				GetProcAddress(ntdll,
				"NtDeviceIoControlFile");
	if (!NtFsControlFile && ntdll)
		NtFsControlFile = (LPFN_NTFSCONTROLFILE)
				GetProcAddress(ntdll,
				"NtFsControlFile");
...
/**
 * ntfs_device_win32_open_handle - open a handle as partition
 * @handle:		handle to open
 * @start_offset:	partition start offset
 * @part_length:	partition length
 * @fd:			win32 file device to return
 * @flags:		unix open status flags
 *
 * Return  0 if o.k.
 ...
 * When fails, fd contents may have not been preserved.
 */
static int ntfs_device_win32_open_handle(HANDLE handle,
		s64 start_offset, s64 part_length, win32_fd *fd, int flags)
{
	if (handle == INVALID_HANDLE_VALUE) {
		/* error */
		errno = ENOENT;
	...
	fd->geo_size = fd->part_length;
	fd->vol_handle = INVALID_HANDLE_VALUE;
	fd->noclose = TRUE; /* IMPORTANT: Don't close the handle when unmount! */
	return 0;
}
 ...
 * If name is in format "(handle:[0-9]:[0-9]:[0-9])" then open a handle
...
static int ntfs_device_win32_open(struct ntfs_device *dev, int flags)
{
	...
	if (!strncmp(dev->d_name, "handle:", 7)) {
		size_t handle_value;
		numparams = sscanf(dev->d_name, "handle:%zu:%lld:%lld", &handle_value, &start_offset, &part_length) + 1;
		handle = (HANDLE)handle_value;
		if (numparams != 4) {
			numparams = -1;
		}
	}
	if (!strncmp(dev->d_name, "file:", 5)) {
		char filename[MAX_PATH];
		numparams = sscanf(dev->d_name, "file:%[^|]|%lld|%lld", filename, &start_offset, &part_length) + 1;
		if (ntfs_device_win32_simple_open_file(filename, &handle, flags,
				FALSE)) {
			/* open error */
			return -1;
	...
	case 3:
		...
		break;
	case 4:
		ntfs_log_debug("win32_open(%s) -> handle %zu, start 0x%llx, length 0x%llx.\n",
				dev->d_name, handle, start_offset, part_length);
		err = ntfs_device_win32_open_handle(handle, start_offset, part_length, &fd,
				flags);
		break;
	default:
...
void hexdump(unsigned char *buf, s64 count) {
	for (s64 i = 0; i < count / 16 + (count % 16 ? 1 : 0); i++) {
		for (s64 j = 0; j < 16; j++) {
			if (i * 16 + j < count) printf("%.2X ", buf[i * 16 + j]);
			else printf("   ");
		}
		printf("| ");
		for (s64 j = 0; j < 16; j++) {
			if (i * 16 + j < count) printf("%c", isprint(buf[i * 16 + j]) ? buf[i * 16 + j] : '.');
			else printf(" ");
		}
		printf("\n");
		if ((i + 1) % 32 == 0) printf("\n");
